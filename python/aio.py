...
async def fetch(frame, session):
    headers = {'X-NCP-APIGW-API-KEY-ID': 'ad7ueq9hal', 'X-NCP-APIGW-API-KEY': 'Ov83nJAvffDEYEhKPDiSt1JAEz9OBkJ9IXZhIgln' }
    files = {'image': frame}
    async with session.post('https://naveropenapi.apigw.ntruss.com/vision/v1/face',headers=headers, json=files) as response:
        print(response)
        return await response.read()

async def get(sem, frame, session):

    async with sem:
        await fetch(frame, session)
...
async def process():
    ...
    async with ClientSession() as session:
        while True:
            ...
            task = asyncio.ensure_future(get(sem, frame, session))
            #서버에 데이터 전송
